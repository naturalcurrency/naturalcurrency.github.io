-- Retry Batch 1 (MEA) with country-prefixed slugs to avoid global slug collisions

-- Remove placeholder entries for MEA countries (rollback-safe)
...
SELECT c.id, division_name,
  CONCAT(c.slug, '-', LOWER(TRIM(BOTH '-' FROM REGEXP_REPLACE(REGEXP_REPLACE(division_name, '''', '', 'g'), '[^a-zA-Z0-9]+', '-', 'g'))))
